...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.3\linewidth]{firewall_image.png}
    \caption{Grey-hat Hacker}
...
Each input consists of one test case, representing a single secure system. The first line contains an integer $n$ ($1 \le n \le 1\,000$), the number of passcodes in the firewall. Each of the next $n$ lines contains one passcode, a positive integer consisting of between $1$ and $4\,300$ digits (inclusive).

\section*{Output}

If every passcode line satisfies the cyclic prefix-divisibility rule defined above, output that the firewall is secure.  \\
